...
popOp(i) = myOsc(freq+freqEnv), myOsc((freq*2.001)+freqEnv)/6, myOsc((freq*3.01)+freqEnv)/10  :> ampEnv
with{
    myOsc(f) = os.phasor(1, f ) * (ma.PI*2) : cos *(0.5); 
    ...
    lfoDepth = hslider("lfoDepth%i", 10, 0, 400, 1);
    lfoDelay = hslider("lfoDelay%i", .4, 0, 5, 0.01) * ma.SR;
    lfoA = hslider("lfoA%i", 0.01, 0.001, 4, 0.001);
    lfoD = hslider("lfoD%i", 0.01, 0.001, 4, 0.001);
    lfoS = hslider("lfoS%i", 0.5, 0, 1, 0.01);
    lfoR = hslider("lfoR%i", 1, 0.001, 4, 0.001);
    lfo = myOsc( en.adsre(lfoA,lfoD,lfoS,lfoR,gate@lfoDelay) * lfoRate );
